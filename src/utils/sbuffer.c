...
void* sbuffer_insert(sbuffer* sb, sbi* sbi, ureg size){
    ...
    ureg free_space_before = ptrdiff(
        cs->start,
        (u8*)cs + sizeof(sbuffer_segment)
    );
    ...
    if(free_space_before + free_space_after >= size){
        ...
        memmove(
            cs->start + used_space_before + size,
            sbi->pos,
            used_space_after
        );
    ...
    if(cs->prev != NULL && used_space_before + free_space_before >= size){
        ...
        ureg prev_free_space_before = ptrdiff(
            cs->prev->start,
            (u8*)(cs->prev) + sizeof(sbuffer_segment)
        );
        if(
            prev_free_space_after + prev_free_space_before
            >= 
            used_space_before
        ){
    ...
    if(cs->next != NULL && used_space_after + free_space_after >= size){
        ureg next_free_space_before = ptrdiff(
            cs->next->start,
            (u8*)cs->next + sizeof(sbuffer_segment)
        );
        ...
        if(
            next_free_space_before + next_free_space_after 
            >= 
            used_space_after
        ){
