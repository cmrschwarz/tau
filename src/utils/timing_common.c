...
freg timespan_get_fmicros(timespan* t){
    return (freg)t->seconds * (freg)microsecs_sec + (freg)t->nanos / (freg)microsecs_nanosec; 
}
freg timespan_get_fmillis(timespan* t){
    return (freg)t->seconds * (freg)millisecs_sec + (freg)t->nanos / (freg)millisecs_nanosec; 
}
...
freg timespan_get_fminutes(timespan* t){
    return (freg)t->seconds / (freg)secs_minute + (freg)t->nanos / (freg)(nanosecs_sec * secs_minute);
}
freg timespan_get_fhours(timespan* t){
    return (freg)t->seconds / (freg)secs_hour + (freg)t->nanos / (freg)(nanosecs_sec * secs_hour);
}
freg timespan_get_fdays(timespan* t){
    return (freg)t->seconds / (freg)secs_day + (freg)t->nanos / (freg)(nanosecs_sec * secs_day);
}
