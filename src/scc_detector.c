...
static inline sccd_node* sccd_get(scc_detector* d, ureg id)
{
...
int sccd_emit_mdg(scc_detector* sccd, sccd_stack_entry* se_curr)
{
    ...
    return tauc_request_resolve_multiple(tc->t, nodes, nodes_end);
}
int sccd_prepare(scc_detector* sccd, mdg_node* n)
{
    scc_detector_housekeep_ids(sccd);
    rwlock_read(&n->lock);
    if (n->stage != MS_AWAITING_DEPENDENCIES) {
    ...
    scc_detector* sccd, mdg_node* n, sccd_node* sn, sccd_stack_entry* next_se)
{
    ...
    list_rit rit;
    list_it_begin(&rit, &n->dependencies);
    rwlock_end_read(&n->lock);
    ...
    se->children_rit = rit;
    se->children_rit_head_sanity_check = rit.head;
    se->mdgn = n;
    se->sn = sn;
    se->deps_count = 0;
    se->scc_elem_count = 1;
...
int sccd_run(scc_detector* sccd, mdg_node* n)
{
    ...
    while (true) {
        ...
        mdg_node* dep;
        while (true) {
            dep = list_rit_prev(&se->children_rit, &n->dependencies);
            if (!dep) break;
            se->deps_count++;
